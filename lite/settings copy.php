<?PHP
	...
	function get_customized()
	{
		...
		{
			$result = mysql_query("SELECT * FROM `display` WHERE `user`=(SELECT `ID` FROM `Users` WHERE `COOKIE`='" . $_COOKIE["QuickLogs"] . "' LIMIT 1)");
			if($result)
			{
				//successful
				$row = mysql_fetch_array($result); //Should be run once
			...
			if ( phpCAS::isAuthenticated() ){
				//no cookie but authenticated, make cookie and then continue process
				...
				//now we have cookie, continue on
				$result = mysql_query("SELECT * FROM `display` WHERE `user`=(SELECT `ID` FROM `Users` WHERE `COOKIE`=" . $_COOKIE["QuickLogs"] . " LIMIT 1)");
				...
				//we arent authenticated or cookied, so we get default set
				return get_default();
			}	
		}
	}
	...
	// no SSL validation for the CAS server, remove when done debugging, if to be used in production
	...
	$admin=false;
	if (phpCAS::isAuthenticated() AND isset($_COOKIE["QuickLogs"]))
	{
		$result = mysql_query("SELECT `type` FROM `Users` WHERE `COOKIE`='" . $_COOKIE["QuickLogs"] . "' LIMIT 1");
		if($result)
		{
			//successful
			$row = mysql_fetch_array($result); //Should be run once
	...
	if ($settings && isset($_COOKIE["QuickLogs"]))
	{
		...
		<div id="main">
			...
			if ($admin)
			{
				...
				echo				"</select></div></a>
						...
						$result = mysql_query("SELECT `index`,`problem` FROM `Types` LIMIT 0, 100"); //limit just in case
						//error_reporting(E_ALL);
						$OPTIONS = array();
						$OPTIONS_ID = array();
						if($result)
						...
						for($i = 0; $i < count($OPTIONS); $i++)
						{
							echo "<div id='selection_of_options" . ($i % 2) . "'>";
