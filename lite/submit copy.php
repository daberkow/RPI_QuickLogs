<?PHP
	if(isset($_REQUEST['Task_ID']))
	{
		...
		if (isset($_COOKIE["QuickLogs"]))
		{
			$result = mysql_query("INSERT INTO `QuickLogs`.`Logs` (`timestamp`, `type`, `userid`) VALUES (CURRENT_TIMESTAMP, " . $_REQUEST['Task_ID'] . ", (SELECT `ID` FROM `Users` WHERE `COOKIE`='" . $_COOKIE["QuickLogs"] . "' LIMIT 1))");
		}else{
	...
	else
	{
		if (isset($_REQUEST['Box1']) AND isset($_REQUEST['Box2']) AND isset($_REQUEST['Box3']) AND isset($_REQUEST['Box4']) AND isset($_REQUEST['Box5']) AND isset($_REQUEST['Box6'])  AND isset($_REQUEST['Box7'])  AND isset($_REQUEST['Box8']))
		{
			...
			if(isset($_REQUEST['Admin']) AND isset($_COOKIE["QuickLogs"]))
			{
				$admin_check = mysql_query("SELECT `type` FROM `Users` WHERE `COOKIE`='" . $_COOKIE["QuickLogs"] . "'");
				if ($admin_check)
				{
			...
			if (isset($_COOKIE["QuickLogs"]) AND !isset($_REQUEST['Admin'])) // this should be true
			{
				$result = mysql_query("UPDATE `QuickLogs`.`display` SET `Box1`='" . $_REQUEST['Box1'] . "', `Box2`='" . $_REQUEST['Box2'] . "', `Box3`='" . $_REQUEST['Box3'] . "', `Box4`='" . $_REQUEST['Box4'] . "', `Box5`='" . $_REQUEST['Box5'] . "', `Box6`='" . $_REQUEST['Box6'] . "', `Box7`='" . $_REQUEST['Box7'] . "', `Box8`='" . $_REQUEST['Box8'] . "' WHERE `user`=(SELECT `ID` FROM `Users` WHERE `COOKIE`='" . $_COOKIE['QuickLogs'] . "' LIMIT 1)");
				if ($result)
				{
